...
| SCR-F05-01 | Handover & Return | F05 | `asset.checkout` và/hoặc `asset.checkin` | US-F05-01, US-F05-03 | Một screen có hai tab `Pending Handover` và `Pending Return`; mỗi tab chỉ gọi queue API tương ứng khi được mở. | Navigation; Approval Detail link; Dashboard. | Xem từng asset; xác nhận handover; xác nhận return thường/hỏng; retry và pagination. | BOR-07, BOR-09 | IMPLEMENTED |
| SCR-F05-02 | Borrowing Activity | F05 | Nhân viên hoặc permission xem lịch sử toàn bộ | US-F05-02, US-F05-04, US-F05-05 | Xem asset đang mượn và history theo phạm vi quyền; mở detail của một history để xem request reason, approval, handover và return metadata. | Navigation Borrowing Activity; Dashboard. | Chọn current/history; mở Borrowing Activity Detail; mở asset/request liên quan khi được phép. | BOR-04, BOR-05, BOR-10 | NEEDS_UPDATE |
...
| SCR-VEN-01 | Vendor Management | Shared master/F06 | `vendor.view`; actions theo `vendor.create/update` | US-VEN-01 | Search, phân trang, lọc Active/Inactive và quản lý shared vendor. | Navigation Vendors; issue repair selector không mở màn này. | Create; Edit thông tin và status trong cùng modal; không hiển thị action Delete hoặc Activate/Deactivate riêng; loading/empty/error/forbidden. | Chưa có | NEW |
...
| Handover & Return | Hai tab Pending Handover và Pending Return cho handover, normal return và damaged return. |
...
- Department delete, role delete, permission CRUD và dashboard theo role đều ngoài MVP.
