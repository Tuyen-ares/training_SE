...
- Code/route hiện tại có registration và menu theo role cũ. Đây là implementation/reference cũ, không phải nguồn nghiệp vụ.

## Quyết định frontend đã chốt

- Có **một Dashboard chung** sau đăng nhập, không có Staff/Manager/Admin dashboard riêng.
- Dashboard chỉ hiện widget hoặc shortcut đến nghiệp vụ mà user có permission. Nó không tạo KPI, analytics hoặc workflow mới.
- Permission thực tế là hợp của permission từ mọi role của user; không có hierarchy `Admin > Manager > Staff`.
...
| FD-02 | `AUTH-02` hỗ trợ registration. | Public registration ngoài MVP. |
