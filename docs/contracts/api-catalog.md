...
| API | English name | Tên tiếng Việt | Purpose / Mục đích | Status |
| --- | --- | --- | --- | --- |
| `GET /api/borrow-request-details/review-queue` | List review queue | Xem hàng đợi duyệt | Server-side filter theo `approvalStatus=PENDING` (mặc định), `ALL`, `APPROVED` hoặc `REJECTED`; `ALL` đặt request có detail `PENDING` trước và phân trang theo request. / Lọc server-side theo trạng thái detail, nhóm Pending đứng trước khi xem tất cả. | Updated |
...
| API | English name | Tên tiếng Việt | Purpose / Mục đích | Status |
| --- | --- | --- | --- | --- |
| `GET /api/borrow-request-details/handover-queue` | List pending handover request groups | Xem hàng đợi bàn giao theo phiếu | List request-level groups containing `APPROVED` + `RESERVED` details without a borrow history for `asset.checkout`; pagination/total count requests, not details. / Liệt kê group cấp phiếu chứa các detail đã duyệt, asset reserved chưa bàn giao; phân trang/total tính theo phiếu. | Updated |
| `GET /api/borrow-request-details/handover-queue/:requestId` | Get handover request detail | Xem chi tiết bàn giao theo phiếu | Load one request group for `asset.checkout` so an admin/manager can inspect assets and confirm handover one detail at a time with optional HANDOVER evidence. / Tải context một phiếu để kiểm tra asset và xác nhận từng detail kèm evidence HANDOVER tùy chọn. | New |
...
| `GET /api/borrow-histories/return-queue` | List pending return request groups | Xem hàng đợi hoàn trả theo phiếu | List request-level groups containing unreturned histories for `asset.checkin`; pagination/total count requests, not histories. / Liệt kê group cấp phiếu chứa history chưa trả; phân trang/total tính theo phiếu. | Updated |
| `GET /api/borrow-histories/return-queue/:requestId` | Get return request detail | Xem chi tiết hoàn trả theo phiếu | Load one request group for `asset.checkin` so an admin/manager can inspect histories and confirm normal or damaged return one history at a time with optional RETURN evidence. / Tải context một phiếu để kiểm tra history và xác nhận trả thường/hỏng từng history kèm evidence RETURN tùy chọn. | New |
...
| `GET /api/borrow-histories/activity/me` | List my request-grouped borrowing activity | Xem activity mượn theo phiếu của tôi | Return current or returned history grouped by borrow request; pagination/total count requests, not histories. Requires `borrow_history.view_own`. / Nhóm lịch sử hiện tại/đã trả theo phiếu; phân trang/total tính theo phiếu. | New |
| `GET /api/borrow-histories/activity` | List all request-grouped borrowing activity | Xem activity mượn theo phiếu toàn bộ | Return company-wide current or returned history grouped by borrow request. Requires `borrow_history.view_all`; no group status is returned. / Nhóm lịch sử toàn công ty theo phiếu, không trả status cấp nhóm. | New |
...
- SMS, mobile push and scheduled notifications. SMTP email is asynchronous and adds no public API. / SMS, push mobile và thông báo theo lịch; SMTP email không thêm public API.
