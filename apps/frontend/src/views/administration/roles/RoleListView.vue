...
<template>
  <WorkspaceLayout>
    ...
    <main class="admin-page bigin-page-container">
      ...
      <AppTable v-else class="role-table" :columns="columns" :data-source="filteredRoles" :loading="loading" row-key="id" empty-description="No roles found">
        ...
        <template #mobileRow="{ record }">
          <div class="role-mobile-row"><div class="role-name"><strong>{{ record.name }}</strong><a-tag v-if="record.isSystem" class="system-tag">System</a-tag></div><div class="role-mobile-meta"><span>{{ record.permissionCount }} permissions · {{ record.userCount }} users</span><a-button v-if="canView" class="bigin-touch-target" type="link" @click="router.push({ name: 'role-detail', params: { id: record.id } })">Open</a-button></div></div>
        </template>
      </AppTable>
    </main>
  </WorkspaceLayout>
...
.role-table { background: var(--bigin-surface-panel); border: 1px solid var(--bigin-border-secondary); border-radius: 8px; overflow: hidden; }.role-name { align-items: center; display: flex; gap: 8px; }.system-tag { background: var(--bigin-surface-inset); border-color: var(--bigin-border-default); color: var(--bigin-text-secondary); margin-inline-end: 0; }.role-mobile-row { display: grid; gap: 12px; }.role-mobile-meta { align-items: center; color: var(--bigin-text-secondary); display: flex; justify-content: space-between; gap: 8px; font-size: 12px; }
