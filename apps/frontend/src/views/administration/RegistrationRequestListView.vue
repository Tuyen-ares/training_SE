...
import { listRegistrationRequests } from '../../services/registration.service'
    ...
    <main class="admin-page">
      ...
      <section class="request-surface">
        <div class="filter-row" aria-label="Registration request filters">
          ...
          <a-button type="text" aria-label="Refresh registration requests" :loading="loading" @click="loadRequests">
        ...
        <a-table v-else class="request-table" :columns="columns" :data-source="pageData.items" :loading="loading" row-key="id" :pagination="false" :scroll="{ x: 1005 }">
          <template #bodyCell="{ column, record }">
            ...
            <template v-else-if="column.key === 'action'">
              <div class="action-cell">
                ...
                >Review</a-button>
                <a-button v-else type="link" size="small" @click="router.push({ name: 'registration-request-detail', params: { id: record.id } })">View</a-button>
              </div>
            </template>
          </template>
        ...
        <footer class="table-footer">
          ...
          <a-pagination v-if="pageData.total > pageData.pageSize" :current="pageData.page" :page-size="pageData.pageSize" :total="pageData.total" :show-size-changer="false" show-less-items @change="changePage" />
        </footer>
      </section>
    </main>
  </WorkspaceLayout>
...
<style scoped>
.admin-page { margin: 0 auto; max-width: 1320px; padding: 28px 36px 48px; }.page-heading { margin-bottom: 24px; }.page-heading h1 { font-size: 22px; margin: 12px 0 5px; }.page-heading p { color: var(--bigin-text-muted); margin: 0; }.request-surface { background: var(--bigin-surface-panel); border: 1px solid var(--bigin-border-secondary); border-radius: 8px; overflow: hidden; }.filter-row { align-items: center; display: flex; gap: 8px; padding: 14px 16px 0; }.request-search { max-width: 360px; }.pending-summary { color: var(--bigin-text-secondary); font-size: 13px; margin-left: auto; }.status-tabs { padding: 0 16px; }.status-tabs :deep(.ant-tabs-nav) { margin: 0; }.request-table { border-radius: 0; }.applicant { display: grid; gap: 2px; }.applicant small { color: var(--bigin-text-muted); font-size: 12px; }.action-cell { align-items: center; display: flex; justify-content: center; width: 100%; }.review-action { background: var(--bigin-color-primary); border-color: var(--bigin-color-primary); }.review-action:hover { background: var(--bigin-color-primary-hover); border-color: var(--bigin-color-primary-hover); }.page-alert { margin: 16px; }.table-footer { align-items: center; border-top: 1px solid var(--bigin-border-secondary); color: var(--bigin-text-muted); display: flex; font-size: 12px; justify-content: space-between; min-height: 52px; padding: 8px 16px; }.request-table :deep(.ant-table-thead > tr > th) { background: var(--bigin-surface-subtle); color: var(--bigin-text-secondary); font-size: 12px; font-weight: 600; padding: 12px 16px; }.request-table :deep(.ant-table-tbody > tr > td) { padding: 12px 16px; }
@media (max-width: 640px) { .admin-page { padding: 16px; }.filter-row { align-items: stretch; flex-wrap: wrap; }.request-search { max-width: none; width: 100%; }.pending-summary { margin-left: 0; }.table-footer { align-items: flex-start; flex-direction: column; gap: 8px; padding-block: 12px; } }
</style>
