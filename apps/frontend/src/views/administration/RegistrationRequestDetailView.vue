...
import { approveRegistrationRequest, getRegistrationRequest, rejectRegistrationRequest } from '../../services/registration.service'
...
<template>
  <WorkspaceLayout>
        ...
        <div class="detail-grid">
          ...
          <section class="card decision-card">
            ...
            <template v-if="isPending">
              ...
              <div class="actions"><a-button danger @click="rejectOpen = true">Reject</a-button><a-button type="primary" class="primary-action" :loading="submitting" @click="approve">Approve & Create User</a-button></div>
            </template>
    ...
    <a-modal v-model:open="rejectOpen" title="Reject registration request" ok-text="Reject" ok-type="danger" :confirm-loading="submitting" @ok="reject"><p>The password hash will be cleared and the applicant can submit a new request later.</p><a-textarea v-model:value="form.rejectionReason" :maxlength="1000" :rows="4" placeholder="Optional rejection reason" show-count /></a-modal>
  </WorkspaceLayout>
</template>

<style scoped>
.detail-page { margin: 0 auto; max-width: 1180px; padding: 24px 28px 48px; }.page-heading { align-items: flex-end; display: flex; justify-content: space-between; }.page-heading h1 { font-size: 22px; margin: 10px 0 4px; }.page-heading p { color: var(--bigin-text-muted); margin: 0; }.alert { margin-top: 16px; }.detail-grid { align-items: start; display: grid; gap: 20px; grid-template-columns: minmax(0, 1fr) minmax(340px, .78fr); margin-top: 18px; }.detail-main { display: grid; gap: 20px; }.card { background: var(--bigin-surface-panel); border: 1px solid var(--bigin-border-secondary); border-radius: 8px; padding: 24px; }.card h2 { font-size: 16px; margin: 0 0 18px; }.card label { display: grid; gap: 7px; margin-bottom: 16px; }.card label > span { font-size: 13px; font-weight: 600; }.card label b { color: var(--bigin-color-error); }.card label small { color: var(--bigin-text-muted); font-size: 12px; font-weight: 400; }.impact-copy { color: var(--bigin-text-secondary); margin: 0 0 10px; }.impact-copy:last-child { margin-bottom: 0; }.impact-copy--fallback { background: var(--bigin-surface-warning); border: 1px solid var(--bigin-border-warning); border-radius: 6px; color: var(--bigin-color-warning-text); padding: 10px 12px; }.decision-card { position: sticky; top: 18px; }.actions { border-top: 1px solid var(--bigin-border-secondary); display: flex; gap: 10px; justify-content: flex-end; margin: 24px -24px -24px; padding: 14px 24px; }.primary-action { background: var(--bigin-color-primary); }
@media (max-width: 760px) { .detail-page { padding: 16px; }.detail-grid { grid-template-columns: 1fr; }.decision-card { position: static; } }
</style>
